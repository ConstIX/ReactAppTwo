...
type DataPost = { title: string, text: string, newImage: any, oldImage: any }
...
const EditPost = () => {
   ...
   const [dataPost, setDataPost] = React.useState<DataPost>({ title: '', text: '', oldImage: '', newImage: '' })

   const handleSubmit = () => {
      try {
         const updatedPost = new FormData()
         updatedPost.append("title", dataPost.title.length === 0 ? 'Default' : dataPost.title)
         updatedPost.append("text", dataPost.text.length === 0 ? 'Default text' : dataPost.text)
         updatedPost.append('id', id as any)
         updatedPost.append('image', dataPost.newImage)
   ...
   React.useEffect(() => {
      axios.get(`/posts/${id}`)
         .then(res => setDataPost({ ...dataPost, title: res.data.title, text: res.data.text, oldImage: res.data.imgUrl }))
            ...
            <form onSubmit={(e: React.FormEvent<HTMLFormElement>) => e.preventDefault()} className="add-post__form">
               ...
               <div className="add-post__input">
                  <div className="add-post__item">
                     ...
                     <label className='add-post__btn btn btn_i'>
                        <span>Add Image</span>
                        <input onChange={(e: any) => setDataPost({ ...dataPost, newImage: e.target.files[0], oldImage: '' })} type="file" className="input" />
                     </label>
                  </div>
                  <div className="add-post__image">
                     {dataPost.oldImage && <img src={`http://localhost:3005/${dataPost.oldImage}`} alt={dataPost.oldImage.name} />}
                     {dataPost.newImage && <img src={URL.createObjectURL(dataPost.newImage)} alt={dataPost.newImage.name} />}
                  </div>
               </div>
